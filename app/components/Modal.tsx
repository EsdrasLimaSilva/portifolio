...
const ModalButton = ({text, targetId, hideModal}: {text: string; targetId: string; hideModal: () => void}) => {
    const goTo = () => {
        document.getElementById(targetId)?.scrollIntoView();
        hideModal();
    }

    return <button type="button" className="hover:translate-x-2 transition-all" onClick={goTo}>{text}</button>
}
